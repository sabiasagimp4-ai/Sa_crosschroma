...
| ステップ数 | 4 | 変位を何回に分けて進めるか。増やすほど滑らかになるが、そのぶん重くなる（移動量は変わらない） |
...
1画素あたりのサンプリング数は、ざっくり **`(8 × ステップ数 + 3 + フィルタ24) × 反復回数`**。

- ぼかしも膨張・収縮も0%のときは、フィルタ処理ごとシェーダー内でスキップされる（36タップ→12タップ）
- ステップ数は1歩ごとに3チャンネル分の勾配を取り直すので、増やすとそのまま比例して重くなる。
...
- アルゴリズム（変位・ステップ分割・反復・回転・変調・アルファの扱い）は、
  CPUリファレンス実装に対する29件のテストと、上のサンプル画像のレンダリングで確認している
